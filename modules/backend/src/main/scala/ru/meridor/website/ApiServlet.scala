...
import ru.meridor.website.util.{ValidationSupport, PropertiesFileSupport}
import org.scalatra.json._
import java.util.Date
import ru.meridor.website.export.{ServiceListPDFWriter, ExportSupport}
import ru.meridor.website.notification.NotificationSupport
import org.json4s._
import javax.servlet.http.HttpServletRequest
import ru.meridor.website.db.entities.StoredCalculation
import org.json4s.jackson.JsonMethods
import scala.Some
import org.scalatra.ScalatraServlet
import ru.meridor.website.export.reader.{ServicesList, ServicesListReader}
import ru.meridor.website.processing.OrderFormAdditionalData
import ru.meridor.website.util.RequestUtils._
...
class ApiServlet extends ScalatraServlet
  ...
  private def processOrderRequest(order: Order): Response = if (order.isValid) {
    import ru.meridor.website.db.entities.Person
  ...
  private def sendSMS(phoneNumber: Long, msg: String, warnAboutLowFunds: Boolean = false): Boolean = {
    import ru.meridor.website.notification.entities.{SMSNotification, LowFundsEvent}
  ...
  private def processPdfRequest(exportRequest: ServiceExportRequest) = if (exportRequest.isValid){
    import ru.meridor.website.export.Job
